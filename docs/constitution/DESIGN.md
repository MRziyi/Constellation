...
**Version**: v0.8 (frozen 2026-05-24); **superseded in part by [AGENT-ARCHITECTURE-V2.md](../server/AGENT-ARCHITECTURE-V2.md) as of 2026-05-25**
**Status**: 设计阶段 → Phase 1 + Phase 2 + R-3 + Phase 3a Web Console + Phase 5 v2 architecture pivot 都已实现
**关联文档**: [SOURCE-OF-TRUTH.md](SOURCE-OF-TRUTH.md) · **[AGENT-ARCHITECTURE-V2.md](../server/AGENT-ARCHITECTURE-V2.md)** · [INTERFACE-CONTRACTS.md](../server/INTERFACE-CONTRACTS.md) · [COMPONENT-DESIGN.md](../server/COMPONENT-DESIGN.md) · [DATA-MODEL.md](../server/DATA-MODEL.md) · [UI-UX.md](../glass/UI-UX.md) · [CORTEX-ROUTER-PROMPT.md](../server/CORTEX-ROUTER-PROMPT.md) · [TOOL-ADAPTERS.md](../server/TOOL-ADAPTERS.md) · [twin-seed/](https://github.com/MRziyi/Constellation-Server/tree/main/twin-seed) · [IMPLEMENTATION-PLAN.md](../roadmap/IMPLEMENTATION-PLAN.md) · [USE-CASE-AUDIT.md](../roadmap/USE-CASE-AUDIT.md) · [TOOL-IDEAS.md](../roadmap/TOOL-IDEAS.md) · [halo-ring-plugin-protocol.md](../cross-device/halo-ring-plugin-protocol.md) · [Doc/ui-mockup.html](../assets/ui-mockup.html) · [Doc/brand/](../brand/)
...
- 每次 Trigger 进 Cortex → 读 Twin 拼 context pack（Phase 2 现状: eager-load `identity.md` + 所有 `skills/*.md` + 所有 `people/core/*.md`；Phase 7 升级 two-pass per [DATA-MODEL §11](../server/DATA-MODEL.md)）
- 每次 Execute 完 → 写 receipt 到 Twin + append CHANGELOG.md。**Multi-step 任务每 step 都写 `[step N]` 前缀 receipt**，最终 step 写 final receipt
- 每次 user override / correct / 满意迭代 → Implicit Learning Loop 异步提炼 → 写入 `Twin/skills/` (参 [DATA-MODEL §9](../server/DATA-MODEL.md))
...
| # | 问题 | 计划归宿 |
|---|---|---|
| OQ-1 | v1 必须接入的具体 Mac 本地工具列表 | ✓ 已拍 → [COMPONENT-DESIGN §2.4](../server/COMPONENT-DESIGN.md) |
| OQ-2 | Insight Engine 的扫描频率 / 信号源细节 | [COMPONENT-DESIGN §1.5](../server/COMPONENT-DESIGN.md) + #6 Data Model |
| OQ-3 | Glass HUD 视觉语言 | ✓ 已落地 → [Doc/ui-mockup.html](../assets/ui-mockup.html) + [UI-UX.md](../glass/UI-UX.md) |
| OQ-4 | Twin 数据模型细节 | ✓ 已落地 → [DATA-MODEL.md](../server/DATA-MODEL.md) |
| OQ-5 | 长任务监控的 polling vs subscription 机制 | #4 Component Design tuning |
| OQ-6 | Voice 唤醒触发的具体协议 | ✓ 已落地 → [Doc/ui-mockup.html §3](../assets/ui-mockup.html) (Voice Invoke = LP+SWIPE_UP, VAD-stop) |
...
- **Companion**: [INTERFACE-CONTRACTS.md](../server/INTERFACE-CONTRACTS.md) · [COMPONENT-DESIGN.md](../server/COMPONENT-DESIGN.md) · [DATA-MODEL.md](../server/DATA-MODEL.md) · [UI-UX.md](../glass/UI-UX.md) · [CORTEX-ROUTER-PROMPT.md](../server/CORTEX-ROUTER-PROMPT.md) · [TOOL-ADAPTERS.md](../server/TOOL-ADAPTERS.md) · [twin-seed/](https://github.com/MRziyi/Constellation-Server/tree/main/twin-seed) · [IMPLEMENTATION-PLAN.md](../roadmap/IMPLEMENTATION-PLAN.md) · [TOOL-IDEAS.md](../roadmap/TOOL-IDEAS.md) · [halo-ring-plugin-protocol.md](../cross-device/halo-ring-plugin-protocol.md) · [Doc/ui-mockup.html](../assets/ui-mockup.html) · [Doc/brand/](../brand/)
...
| v0.4 | P1 重定义为"AI 应该怎么作为我做事"的指令集 (Twin 是 Living User Spec)；preferences/ → skills/；人脸识别归 Tool Agent (本地模型，撤销之前归 Cortex 的决议)；Twin 去 git，改 CHANGELOG.md；新增 Implicit Learning Loop 作为 v1 能力；OQ-4 关闭 (移到 [DATA-MODEL.md](../server/DATA-MODEL.md)) |
| v0.5 | #8 UI/UX 落地：[Doc/ui-mockup.html](../assets/ui-mockup.html) + [UI-UX.md](../glass/UI-UX.md)；继承 Halo Ring 视觉语言；2 个 global gesture (Quick Shortcut + Voice Invoke)；HUD options 改 Send/Feedback（不再 Reject）；新增 Feedback Loop 迭代机制（[INTERFACE-CONTRACTS §1.5](../server/INTERFACE-CONTRACTS.md)）；OQ-3/OQ-6 关闭 |
| v0.6 | mockup 重构为"两类 surface"清晰结构（HUD atom-based + 极简 app 设置 + 强制通用 card 模板 + 加 face recognition 验证模板适配性）；手势/profile 完全推给 Halo Ring；新增 [halo-ring-plugin-protocol.md](../cross-device/halo-ring-plugin-protocol.md) 给 Halo Ring agent；brand mark 落地 [Doc/brand/v1-constellation.svg](../brand/v1-constellation.svg)（asymmetric 8-star asterism，9× 放大自 mockup inline mark）；#9 Implementation Plan 落地 [IMPLEMENTATION-PLAN.md](../roadmap/IMPLEMENTATION-PLAN.md)（9 phases + critical path + risk register） |
| v0.7 | **设计阶段收尾**：G1 [CORTEX-ROUTER-PROMPT.md](../server/CORTEX-ROUTER-PROMPT.md)（system prompt + 7 个 few-shot + 输出 schema）；G2 [TOOL-ADAPTERS.md](../server/TOOL-ADAPTERS.md)（6 adapters × 详细 action catalog）；G3 [twin-seed/](https://github.com/MRziyi/Constellation-Server/tree/main/twin-seed) 13 个文件（ready-to-deploy Twin 种子）；IMPLEMENTATION-PLAN bumped v0.1→v0.2（hardware no longer blocking；Mac-first → Android-phone debug → Rokid deploy 三段式；新增 §8 Greenlight checklist） |
...
| C-7 Twin = markdown + skill-style | §2 P1 + §3.2 + [INTERFACE-CONTRACTS §4](../server/INTERFACE-CONTRACTS.md) | ✓ |
