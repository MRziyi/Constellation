...
| **C-65** | **拍照落盘 twin + vision 简介 → memo（UC1 闭环）**. 带 image 的 complex turn：Cortex 把 b64 写到 `twin/memos/assets/img-<eid>.jpg`（已握字节，比塞 130KB b64 进 brief 便宜）+ **跑一次 vision_describe 简介**（detail 按意图，CC 看不了 JPEG 故必须代劳）→ 把相对路径 + 简介一起塞进 brief 的 PHOTO 段，提示 agent 写 `他的笔记 + 简介 + ![](assets/…)`. **brief R1 放开**：写 `~/constellation/twin/` 下的 memo/笔记（他自己的草稿区、他开口要时）**直接 Write、免 propose**；只有 mail/calendar/reminders/imessage + 写别处仍走 actions[] 审批. | UC1. 服务端管道（persist→vision 简介 229 字→brief）+ R1 放开验过；**agent 真写 memo + 真实照片待真机**（1×1 测试图 CC 合理地拒绝处理）. |
| **C-66** | **语音浏览/续跑历史 session（UC2）**. `session_browser`：列某项目最近 CC session（标题 + **最后一句 user 消息** = "我最后说啥"，纯磁盘读、无 LLM）；语音 pick 解析(`#N`/第N个/EN+ZH **+ 按标题/话题模糊匹配** "the gesture one"/"接着能耗那个"——token 重叠 + 子串信用，CJK 子串计 1.0，best 须 ≥1 且严格压过亚军、否则弃权回退 #N)；**live 检测**（ps 匹配真实 `--resume/--session-id <uuid>` 的 claude 进程）. **只 resume idle session**——live 的（在 VS Code 里活着）只读展示，绝不二次 `--resume` 同一 jsonl. 续跑走既有 `claude_code.agent` + `resume_cc_session_id`. **修了既有 bug**：`--resume` 新进程会重弹 bypass 安全门，旧码在 resume 时跳过确认 → 卡死；改为两路都轮询确认. | Zack 用例 + "只允许 idle；live 的只展示". 真机：idle resume 跑出 `RESUMED-OK`；VS Code session 被拦. |
